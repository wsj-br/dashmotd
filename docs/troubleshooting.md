...
- **No dashboard after a Debian upgrade:** Debian's `pam_motd` executes `/etc/update-motd.d` before the SSH session's controlling terminal is attached. The `update-motd.d/50-dashmotd` hook therefore must not test `/dev/tty`; update dashmotd and verify the generated file with `env -i PATH=/usr/bin:/bin run-parts /etc/update-motd.d/`.
